...
var cursorCallback, cursorHandler, expressHandler, pluginInterface;
	
cursorCallback = function(req, res, next, err, cursor) {
	var val, cache = false;
	
	if (err || !cursor) {
		next(err);
	} else {
		if (req.headers['X-Cursor-Offset']) {
			val = parseInt(req.headers['X-Cursor-Offset']);
			if (!isNaN(val)) {
				cursor = cursor.skip(val);
			}
		};
		
		if (req.headers['X-Cursor-Limit']) {
			val = parseInt(req.headers['X-Cursor-Limit']);
			if (!isNaN(val)) {
				if (val < cursor.count()) {
					cache = true;
				}
				
				cursor = cursor.limit(val);
			}
		}
		
		// Cursor cache override header
		if (req.headers['X-Cursor-Cache'] === 'never') {
			cache = false;
		} else if (req.headers['X-Cursor-Cache'] === 'always') {
			cache = true;
		}
...
cursorHandler = function(actionFn, req, res, next) {
	...
	actionFn.call(this, req, cursorCallback.bind(null, req, res, next));
};
...
/* Plugin interface factory. This interface is passed to
   plugins 'register' and 'init' methods. */
pluginInterface = function(emitter, web, name, plugin) {
	...
	iface.registerResources = function(resources) {
		Object.keys(resources).forEach(function (resname) {
			...
			// Create model
			if (res.schema) {
				model = mongoose.model(name + '_' + resname, res.schema);
			}
			...
			if (res.routes) {
				res.routes.forEach(function(rt) {
					...
					case 'express':
						...
						router(route, expressHandler.bind(model, name, resname, rt.action, action.code));
						break;
						
					case 'cursor':
						log.debug("Registering route %s for cursor action '%s'", route, rt.action);
						router(route, cursorHandler.bind(model, action.code));
						break;
