...
exports.warnAdmin = function(addr) {
	log.warn('Config-based admin user authenticated from %s', addr);
};


/* Authenticate user with salted password */
exports.authUser = function(user, passalted, salt, callback) {
	var admin = config.web.admin;
	
	if (admin.enabled && admin.user === user) {
		return callback(null, misc.md5(salt + misc.md5(admin.password)) === passalted, true);
	}
...
/* Check if user has been granted a specific right. Can be called as specified
	in the argument list or as hasRight(user, rightsString, callback)
*/
