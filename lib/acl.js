...
exports.Group = Group = mongoose.model('group', groupSchema);
...
exports.hasRight = function(user, realm, model, action, callback) {
	...
	// Config-admin user has all rights
	if (admin.enabled && admin.user === user) {
...
/* List available rights for user */
exports.rightsHandler = function(req, res, next) {
	var user = req.session.userName,
		admin = config.web.admin,
		rights = [];
	...
	Group.find({ users: user }, function(err, groups) {
		if (err) {
			return next(err);
		}
		
		groups.forEach(function(g) {
			g.rights.forEach(function(r) {
