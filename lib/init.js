...
exports.startup = function(basedir, config, callback) {
	async.waterfall([
		...
		function(ctx, callback) {
			...
			web.get('/plugins/statics', function(req, res, next) {
				try {
			...
			web.get('/cursor/:id/seek/:pos', ccache.seekHandler);
			web.get('/cursor/:id/discard', ccache.discardHandler);
			web.get('/cursor/:id', ccache.getHandler);
