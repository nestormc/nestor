...
class WebOutputManager:
    ...
    def render_js_opcodes(self):
        """Render all pending opcodes into a JS script block """
        ...
        for opcode, params, caller in self.ops:
            ...
            if opcode == 'style':
                ...
                pseudo = params[3]
                if pseudo:
                    ops.append('$W.$cssrule("#%s:%s","%s",%s);' % (id, pseudo, prop, val))
                else:
                    ops.append('$W.$("%s").style.%s=%s;' % (id, prop, val))
                ...
                prop = params[1]
                val = self._json_value(params[2])
                ops.append('$W.$("%s").%s=%s;' % (id, prop, val))
                ...
                cid = self._dom_id(child)
                ops.extend([
                    'var c=$W.document.createElement("%s");' % child.tagname,
                    'c.id="%s";' % cid,
                    '$W.$("%s").appendChild(c);' % id
                ])
                ...
                ops.append('if($W.$("%s"))$W.$("%s").removeChild($W.$("%s"));' % (cid, id, cid))
                ...
                ops.extend([
                    'var p=$W.document.createElement("%s");' % popup.tagname,
                    'p.id="%s";' % pid,
                    '$W.document.documentElement.appendChild(p);'
                ])
                ...
                ops.append('if($W.$("%s"))$W.document.documentElement.removeChild($W.$("%s"));' % (pid, pid))
                #self.el_popups[id].remove(popup)
                
            elif opcode == 'swap':
                sibling = params[1]
                sid = self._dom_id(sibling)
                ops.append('$W.$swap($W.$("%s"),$W.$("%s"));' % (id, sid))
                
            elif opcode == 'sched_update':
                interval = params[1]
                ops.append('$W.$scheduler.schedule(%d,"%s");' % (interval, id))
                
            elif opcode == 'class':
                cls = params[1]
                ops.append('$W.$addC($W.$("%s"),"%s");' % (id, cls))
                
            elif opcode == 'unclass':
                cls = params[1]
                ops.append('$W.$remC($W.$("%s"),"%s");' % (id, cls))
                
            elif opcode == 'event':
                event = params[1]
                handlerid = self.handler_id(params[2])
                arg = self._json_value(params[3])
                ops.append('$W.$("%s").%s=function(e){$W.$method(%d,%s);'
                    'e.stopPropagation();};' % (id, event, handlerid, arg))
                    
            elif opcode == 'jsevent':
                event = params[1]
                handler = params[2]
                ops.append('$W.$("%s").%s=%s;' % (id, event, handler))
                
            elif opcode == 'jscode':
                code = params[1].replace('{id}', '"%s"' % id)
                code = code.replace('{this}', '$W.$("%s")' % id)
                ops.append("%s;" % code)
        ...
        if self.fatal:
            ops.append("$W.$fatal(%s);" % self._json_value(self.fatal))
            
        if DEBUG:
            for d in self.debug_msgs:
                ops.append("$W.$debug(%s);" % self._json_value(d))
...
<body>
<!-- <iframe id="NESTOR_SCRIPTS" src="/ui/scripts"></iframe> -->
%s
%s
</body>
</html>
