...
DATABASES = ['nestor', 'objects', 'media']
UPDATE_SCRIPTS = { 
    1: {
        'nestor': """
...
class DBUpdater:
    ...
    def __init__(self, **kwargs):
        for db in DATABASES:
            dbfile = kwargs.get(db, '/var/lib/nestor/%s.db' % db)
    ...
    def get_db_version(self):
        try:
            ...
            return int(self.conns['nestor'].execute(query).fetchall()[0][0])
        except (sqlite3.Error, IndexError):
            return 0
            
    def set_db_version(self, version):
        query = "INSERT OR REPLACE INTO config(key,value) VALUES('version',?)"
        self.conns['nestor'].execute(query, (version,))
        self.conns['nestor'].commit()
