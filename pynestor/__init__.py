...
from .errors import NestorError, DaemonizeError, UnexpectedStopError
...
class Nestor:
    ...
    def _init_db(self, **kwargs):
        ...
        main_db = kwargs.get('main_db', '/var/lib/nestor/nestor.db')
        obj_db = kwargs.get('obj_db', '/var/lib/nestor/objects.db')
        media_db = kwargs.get('media_db', '/var/lib/nestor/media.db')
        
        for dbfile in [main_db, obj_db, media_db]:
            try:
                db = sqlite3.connect(dbfile)
            except sqlite3.Error:
                raise NestorError("Cannot connect to database (%s)" % dbfile)
            else:
                db.close()
    ...
    def _get_logger(self, file_key='', level_key=''):
        ...
        try:
            ...
            raise NestorError("No log file specified")
        if not os.access(logfile, os.F_OK):
            try:
                open(logfile, 'w').close()
            except IOError:
                raise NestorError("Log file cannot be written (%s)" % logfile)
