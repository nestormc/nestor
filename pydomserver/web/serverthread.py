...
class RequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def _do_cover(self, parm, head):
        ...
        mdir = self.server.domserver.config['media.music_dir']
        skinname = parm[0]
        ...
        if not path.startswith("%s/" % mdir) or not os.path.isfile(path):
            staticpath = self.server.domserver.config["web.static_dir"]
    ...
    def _do_web(self, parm, head):
        ...
        web_dir = self.server.domserver.config["web.static_dir"]
    ...
    def _start_session(self):
        ...
        sid = C.get('domserver_sid', None)
        if sid:
            sid = sid.value
        if sid and not self._session_exists(sid):
            sid = None    
            
        expires = int(self.server.domserver.config["web.session_expires"])
        db = self.ds.get_main_db()
        if not sid:
            chost, cport = self.client_address
            sid = hashlib.sha256(
                "domserver:%f:%s:%d" % (time.time(), chost, cport)
            ).hexdigest()
        ...
        C['domserver_sid'] = sid
        C['domserver_sid']['expires'] = time.strftime(
            "%a, %d %b %Y %H:%M:%S +0000",
...
class DomserverHTTPServer(BaseHTTPServer.HTTPServer):

    def __init__(self, host, rqh, domserver, thread):   
    ...
    def get_om(self, sid):
        expires = int(self.domserver.config["web.output_expires"])
        delete = []
...
class HTTPServerThread(Thread):
    ...
    def domserver_run(self):
        host = self.domserver.config["web.host"]
        port = int(self.domserver.config["web.port"])
        
        self.domserver.info("Starting web server, listening on %s:%d" % (host,
            port))
        
        self.https = DomserverHTTPServer((host, port), RequestHandler,
            self.domserver, self)
        ...
        self.domserver.info("Stopped web server")
